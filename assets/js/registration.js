...
var idproof=0,biodata=0;
    function fileCheck(obj) {
        var fileExtension = ['jpeg', 'jpg', 'png', 'gif', 'bmp'];
        if ($.inArray($(obj).val().split('.').pop().toLowerCase(), fileExtension) == -1){
            alertify.error('Image file Only');
            profilepicture=1;}
        else{
    ...
    function idproofcheck(obj){
        var fileExtension = ['pdf'];
        if ($.inArray($(obj).val().split('.').pop().toLowerCase(), fileExtension) == -1){
            alertify.error('PDF file only');
            idproof=1;}
        else{
...
function biodatacheck(obj){
    var fileExtension = ['pdf'];
    if ($.inArray($(obj).val().split('.').pop().toLowerCase(), fileExtension) == -1){
        alertify.error('PDF file only');
       biodata=1;}
    else{
        document.getElementById("biocheck").style.visibility="visible";
       biodata=0;
    }
}
...
function nextPrev(n) {
    ...
    if(currentTab==0 && n==1){
        ...
        if(ms.value=='ms' || child.value=="child" || height.value=="0" || inch.value =="none" || btype.value =="0" || htype.value =="0" || wol.value =="0" || ft.value =="0" || rp.value =="0" || fiqhs.value=="0" || ns.value=="0" || wywm.value=="0"){
            ...
            if(gname.value.length==0 || gphone.value.length==0 || profilephoto.length==0 || idproof.length==0 || biodata.length==0 || profilepicture==1 || idproof==1 || biodata==1){
                alertify.error('Fill All Fields');
